/**
 * partners.fabricacao.profile.js
 *
 * Controller / rota do painel do parceiro fabricação.
 * Responsável por:
 * - validar acesso
...
const { renderFabricacaoProfilePage } = require("./partners.fabricacao.profile.view");

module.exports = function registerFabricacaoProfileRoute(ctx) {
  const {
    app,
    isDev,
    supabase,
  ...
  app.get("/parceiros/perfil/:id", requirePartner, async (req, res, next) => {
    try {
      ...
      if (pErr || !p) {
        console.error("[perfil] erro ao buscar parceiro:", pErr);
        return res.redirect("/parceiros");
      }

      // ✅ IMPORTANTE:
      // Se não for parceiro de fabricação, passa para a próxima rota
      if (String(p.tipo || "").toLowerCase() !== "fabricacao") {
        return next();
      }
      ...
      if (oErr) {
        console.error("[perfil] erro ao buscar pedidos:", oErr);
      }
      ...
      if (walletActErr) {
        console.error("[perfil] erro ao buscar atividades da carteira:", walletActErr);
      }
      ...
      const pedidos_em_fabricacao = orders.filter(
        (x) => x.status === "em_fabricacao" || x.status === "pronto_entrega"
      ).length;
      ...
      const ordersEmFab = orders.filter(
        (x) => x.status === "em_fabricacao" || x.status === "pronto_entrega"
      );
      ...
      const previsaoSeFinalizarPendentes =
        Number(carteira_total_disponivel || 0) +
        pedidos_em_fabricacao * Number(level.commission || 0);
      ...
      // Render
      // =========================
      const html = renderFabricacaoProfilePage({
        p,
        orders,
        wallet,
      ...
      console.error("[partners] perfil erro:", msg, e);
  ...
  function titleFromPartner(partner) {
    ...
    return negocio ? `Perfil — ${negocio}` : "Perfil — Fabricação";
  }

  function titleSafe(s) {
    return String(s || "Perfil — Fabricação");
  }
};