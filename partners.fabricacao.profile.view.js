/**
 * partners.fabricacao.profile.view.js
 *
 * View do painel do parceiro fabricação.
 * Responsável por montar o HTML completo da tela.
 *
 * Depende de:
 * - ./partners.fabricacao.profile.assets
 */
...
} = require("./partners.fabricacao.profile.assets");

module.exports = {
  renderFabricacaoProfilePage,
};

function renderFabricacaoProfilePage(data = {}) {
  const {
    p,
    orders = [],
    wallet = {},
  ...
  const statusLabelLocal = (st) => {
    ...
    if (s === "em_fabricacao") return "Em fabricação";
    if (s === "pronto_entrega") return "Pronto para entrega";
        ...
        <table class="table tableReadable">
          <thead>
            <tr>
              ...
              <th>Total</th>
              <th>Ganho</th>
              ${showActions ? `<th style="width:360px;">Ações</th>` : ``}
            </tr>
          </thead>
          <tbody>
            ${list
              ...
              .map((o) => {
                ...
                const rowText = [
                  ...
                  fmtCli(o),
                  statusLabelLocal(st),
                  o.valor_total != null ? moneyBR(o.valor_total) : "-",
                  ...
                  ? `
                    <div class="rowActions">
                      ${
                        st === "para_aceitar"
                          ? `
                            <button class="btnSmall btnAccept" data-action="setStatus" data-id="${esc(
                              o.id
                            )}" data-status="em_fabricacao" type="button">✅ Aceitar</button>
                            <button class="btnSmall btnReject" data-action="setStatus" data-id="${esc(
                              o.id
                            )}" data-status="recusado" type="button">⛔ Recusar</button>
                          `
                          : st === "em_fabricacao"
                          ? `
                            <button class="btnSmall btnReady" data-action="setStatus" data-id="${esc(
                              o.id
                            )}" data-status="pronto_entrega" type="button">📦 Pedido pronto saindo para entrega</button>
                            <button class="btnSmall btnDelivered" data-action="setStatus" data-id="${esc(
                              o.id
                            )}" data-status="finalizado" type="button">✅ Pedido Entregue</button>
                          `
                          : st === "pronto_entrega"
                          ? `
                            <button class="btnSmall btnDelivered" data-action="setStatus" data-id="${esc(
                ...
                return `
                  <tr data-search="${esc(rowText)}">
                    ...
                    <td>${esc(String(o.valor_total != null ? moneyBR(o.valor_total) : "-"))}</td>
  ...
  const historicoHtml =
    orders.length === 0
      ...
      : `
        <div class="tableTools">
          <input class="tableSearch" data-table-search placeholder="Filtrar histórico por cliente, cidade, status..." />
        </div>
            ...
            <tbody>
              ${orders
                ...
                .map((o) => {
                  ...
                  const totalV = String(o.valor_total != null ? moneyBR(o.valor_total) : "-");
                  const ganhoExibido = String(o.status || "") === "finalizado" ? Number(o.ganho_parceiro || 0) : 0;
                  const st = String(o.status || "");
                  const rowText = [when, tipoTxt, statusLabelLocal(st), cli, totalV, fmtCoins(ganhoExibido)]
                    .join(" ")
                    .toLowerCase();
  ...
  const menuItems = [
    ...
    { id: "recusados", label: `⛔ Recusados ${badge(pedidos_recusados, "warn")}` },
  ...
  const openNow = pedidos_para_aceitar + pedidos_em_fabricacao;
  const dayPct = doneNow + openNow ? Math.round((doneNow / (doneNow + openNow)) * 100) : 0;
  ...
  const sidebarHtml = `
    <div class="sideCardClean">
      ...
      <div class="sideInfoBox">
        ...
        <div class="miniLine">
          <span class="muted">Comissão atual</span>
          <span><b>${esc(fmtCoins(level.commission || 0))}</b></span>
        </div>
        <div class="miniHint muted">Valor por pedido entregue no seu nível atual.</div>
  ...
  const topHero = `
    <section class="heroClean">
      <div class="heroLeft">
        ...
        <h1 class="heroTitle">Bem-vindo ao seu painel de fabricação</h1>
        <p class="heroText">
          Acompanhe seus pedidos, suas moedas, sua evolução e tudo o que importa no dia a dia em um só lugar.
        </p>

        <div class="heroButtons">
          <button class="heroBtn heroBtnPrimary" type="button" data-tab="aceitar">Ver pedidos</button>
      ...
      <div class="heroRight">
        <div class="heroStatCard">
          ...
          <div class="heroStatValue">${esc(fmtCoins(carteira_total_disponivel))}</div>
        </div>
        ...
        <div class="heroMiniStats">
          <div class="heroMiniStat">
            <span>📥</span>
            <div>
              <b>${esc(String(pedidos_para_aceitar))}</b>
              <small>Para aceitar</small>
            </div>
          </div>
  ...
  const resumaoTopo = `
    <section class="wideSection">
      ${sectionLead(
        ...
        "Veja rapidamente o que está em andamento no momento."
      )}

      <div class="panel widePanel">
        <div class="topSummaryGrid">
          <div class="topSummaryCard">
            <div class="topSummaryLabel">Para aceitar</div>
            <div class="topSummaryValue">${esc(String(pedidos_para_aceitar))}</div>
          </div>

          <div class="topSummaryCard">
            <div class="topSummaryLabel">Em fabricação</div>
            <div class="topSummaryValue">${esc(String(pedidos_em_fabricacao))}</div>
          </div>
      ...
      <div class="panel widePanel" data-panel="visao">
        ...
        <div class="kpiGrid">
          <div class="kpiCard kpiCardCoins">
            ...
            <div class="kpiValue" id="walletTotalText">${esc(fmtCoins(carteira_total_disponivel))}</div>
          ...
          <div class="kpiCard">
            ...
            <div class="kpiSub">Comissão atual: ${esc(fmtCoins(level.commission || 0))}</div>
          </div>
          ...
          <div class="kpiCard">
            ...
            <div class="kpiSub">Pedidos já concluídos</div>
          </div>
        </div>
        ...
        <div class="note noteReadable">
          ...
          <div class="muted">Mantenha seus pedidos atualizados para acompanhar sua evolução.</div>
        </div>
        ...
        <div class="split">
          ...
          <div class="miniCard miniWarn">
            <div class="miniTitle">🚚 Em andamento</div>
            <div class="miniValue">${pedidos_em_fabricacao}</div>
            <div class="muted">Pedidos em produção ou entrega</div>
          </div>

          <div class="miniCard">
            <div class="miniTitle">📈 Previsão</div>
            <div class="miniValue">${esc(fmtCoins(previsaoSeFinalizarPendentes))}</div>
            <div class="muted">Se concluir os pedidos atuais</div>
          </div>
        </div>
        ...
        <div class="split">
          <div class="miniCard">
            <div class="miniTitle">📦 Total de pedidos</div>
            <div class="miniValue">${totalPedidos}</div>
            <div class="muted">Todas as movimentações deste parceiro</div>
          </div>

          <div class="miniCard">
            <div class="miniTitle">⭐ Média por entrega</div>
          ...
          <div class="miniCard miniBad">
            ...
            <div class="muted">Pedidos recusados por você</div>
          </div>
        </div>
        ...
        <div class="cashGrid">
          <div class="cashCard">
            <div class="cashCardHead">
              ...
              <div class="softPill">${esc(now.toLocaleDateString("pt-BR", { month: "long", year: "numeric" }))}</div>
            </div>
            ...
            <div class="resumeList">
              ...
              <div class="resumeItem resumeItemTotal">
                ...
                <b>${esc(String(pedidosMesAtualFinalizados))} / ${esc(String(metaMensalFinalizados))} pedidos</b>
              </div>
            </div>
            ...
            <div class="note noteReadable">
              ...
              <div class="bar barBig"><div class="barFill" style="width:${esc(String(metaMensalPct))}%;"></div></div>
            </div>
          </div>
      ...
      <div class="panel widePanel" data-panel="caixa">
        <div class="panelHead panelHeadStack">
          <div>
            ...
            <div class="muted">
              Seu saldo é formado pelas moedas liberadas quando um pedido vira <b>Finalizado</b>,
        ...
        <div class="walletHero walletHeroReadable">
          <div class="walletHeroMain">
            ...
            <div class="walletBig" id="walletBigText">${esc(fmtCoins(carteira_total_disponivel))}</div>
          ...
          <div class="walletHeroSide">
            <div class="walletMiniCard">
              ...
              <div class="walletMiniText">Comissão por pedido entregue: <b>${esc(fmtCoins(level.commission || 0))}</b></div>
            </div>
        ...
        <div class="kpiGrid kpiGridThree">
          <div class="kpiCard">
          ...
          <div class="kpiCard">
            ...
            <div class="kpiValue">${esc(fmtCoins(caixa_total))}</div>
            <div class="kpiSub">Moedas já registradas nos pedidos</div>
          </div>

          <div class="kpiCard">
            <div class="kpiTitle">Comissão vigente</div>
            <div class="kpiValue">${esc(fmtCoins(level.commission || 0))}</div>
            <div class="kpiSub">Por pedido entregue no nível ${esc(level.name || "Bronze")}</div>
          </div>
        </div>
        ...
        <div class="cashCard">
            <div class="cashCardHead">
              <div>
                ...
                <div class="muted">Seu nível melhora a recompensa do check-in e da comissão.</div>
              </div>
    ...
    level.name || "Bronze"
            ...
            <div class="tierGrid">
              <div class="tierBox tier-bronze ${level.key === "bronze" ? "isCurrent" : ""}">
                ...
                <div class="tierText">Comissão: 28 moedas</div>
              </div>
              ...
              <div class="tierBox tier-prata ${level.key === "prata" ? "isCurrent" : ""}">
                ...
                <div class="tierText">Comissão: 30 moedas</div>
              </div>
              ...
              <div class="tierBox tier-ouro ${level.key === "ouro" ? "isCurrent" : ""}">
                ...
                <div class="tierText">Comissão: 32 moedas</div>
              </div>
            </div>
          ...
          <div class="cashCard">
            ...
            <div class="resumeList">
              <div class="resumeItem">
                ...
                <b id="baseCoinsText">${esc(fmtCoins(caixa_total))}</b>
              </div>
              <div class="resumeItem">
                <span>Bônus + compras</span>
                <b id="bonusCoinsText">${esc(fmtCoins(wallet_bonus + wallet_purchased))}</b>
              </div>
              <div class="resumeItem">
                <span>Solicitações de saque</span>
                <b id="withdrawCoinsText">${esc(fmtCoins(wallet_withdrawn))}</b>
              </div>
              <div class="resumeItem resumeItemTotal">
                <span>Disponível no painel</span>
                <b id="availableCoinsText">${esc(fmtCoins(carteira_total_disponivel))}</b>
              </div>
            </div>
          </div>
  ...
  const tabAceitar = `
    <section class="wideSection" id="section-aceitar">
      ${sectionLead(
        "Pedidos",
        "Pedidos para aceitar",
        "Aceite para iniciar a produção ou recuse para não pegar esse pedido.",
        `<div class="leadPill">${esc(String(pedidos_para_aceitar))} aguardando</div>`
      )}

      <div class="panel widePanel" data-panel="aceitar">
  ...
  const tabEmFab = `
    <section class="wideSection" id="section-emf">
      ${sectionLead(
        "Produção",
        "Pedidos em fabricação",
        "Acompanhe os pedidos em andamento e atualize o status conforme o avanço.",
        `<div class="leadPill">${esc(String(pedidos_em_fabricacao))} em andamento</div>`
      )}

      <div class="panel widePanel" data-panel="emf">
  ...
  const tabFinalizados = `
    <section class="wideSection" id="section-finalizados">
      ${sectionLead(
        "Entregas",
        "Pedidos finalizados",
        "Veja os pedidos que já foram concluídos.",
        `<div class="leadPill">${esc(String(pedidos_finalizados))} concluídos</div>`
      )}

      <div class="panel widePanel" data-panel="finalizados">
  ...
  const tabRecusados = `
    <section class="wideSection" id="section-recusados">
      ${sectionLead(
        "Consulta",
        "Pedidos recusados",
        "Consulte os pedidos que você decidiu não aceitar.",
        `<div class="leadPill">${esc(String(pedidos_recusados))} recusados</div>`
      )}

      <div class="panel widePanel" data-panel="recusados">
      ...
      <div class="panel widePanel" data-panel="como">
        <div class="steps">
          <div class="step">
            ...
            <div>
              <b>Chega pedido</b>
              <div class="muted">Ele entra em <b>Para aceitar</b>.</div>
            </div>
          </div>
          ...
          <div class="step">
            ...
            <div>
              <b>Você escolhe</b>
              <div class="muted">✅ Aceitar ou ⛔ Recusar.</div>
            </div>
          </div>
          ...
          <div class="step">
            ...
            <div>
              <b>Produz</b>
              <div class="muted">Na aba <b>Em fabricação</b>, avance o status do pedido.</div>
            </div>
          </div>
          ...
          <div class="step">
            ...
            <div>
              <b>Entrega</b>
              <div class="muted">Ao marcar <b>Pedido Entregue</b>, ele vai para <b>Finalizados</b>.</div>
            </div>
          </div>
          ...
          <div class="step">
            ...
            <div>
              <b>Ganhos</b>
              <div class="muted">Os pedidos finalizados alimentam o seu caixa e sua evolução.</div>
            </div>
          </div>
        </div>
      </div>
  ...
  const tabHistorico = `
    <section class="wideSection" id="section-historico">
      ${sectionLead(
        ...
        "Consulte os pedidos mais recentes em um só lugar."
      )}
...
.navBtnClean .badge{
  margin-left:8px;
    ...
    .btnAccept{
      background:#eefcf4;
      border-color:rgba(67,160,91,.22);
      color:#176b2c;
    }
    ...
    .btnDelivered{
      background:#f0fff7;
      border-color:rgba(66, 184, 131, .22);
      color:#12734b;
    }
 ...
 @media (max-width: 1180px){
  ...
  return `
    <style>${FABRICACAO_PROFILE_CSS()}${extraLayoutCss}</style>
  ...
  <div class="layoutGrid">
    ...
    <script>
      ${buildFabricacaoProfileJS({
        ...
        commissionNow: level.commission,
        walletState: {
