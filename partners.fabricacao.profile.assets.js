/**
 * partners.fabricacao.profile.assets.js
 *
 * Assets do painel do parceiro fabricação:
...
module.exports = {
  FABRICACAO_PROFILE_CSS,
  buildFabricacaoProfileJS,
};

function FABRICACAO_PROFILE_CSS() {
  return `
:root{
...
function buildFabricacaoProfileJS(opts = {}) {
  ...
  const commissionNow = Number(opts.commissionNow || 28);
  ...
  const pixState = normalizePixState(opts.pixState || {});
...
(function(){
  ...
  function getTargetTabForStatus(status){
    if(status === 'em_fabricacao' || status === 'pronto_entrega') return 'emf';
    if(status === 'finalizado') return 'finalizados';
    if(status === 'recusado') return 'recusados';
    return getActiveTab();
  }

  function forceReloadToTab(tab){
  ...
  function ensureEmptyState(panel){
    ...
    const visibleRows = Array.from(tbody.querySelectorAll('tr')).filter(function(tr){
      return tr.style.display !== 'none';
    });

    if(visibleRows.length > 0) return;

    const existing = panel.querySelector('.empty');
    if(existing) return;
  ...
  function cleanupEmptyState(panel){
    if(!panel) return;
    ...
    const tbody = panel.querySelector('tbody');
    const visibleRows = tbody ? Array.from(tbody.querySelectorAll('tr')).filter(function(tr){
      return tr.style.display !== 'none';
    }) : [];

    if(visibleRows.length > 0) empty.remove();
  }

  function removeRowFromUI(btn){
  ...
  function lockRow(btn, loading){
    const row = btn ? btn.closest('tr') : null;
    if(!row) return;

    row.style.opacity = loading ? '.55' : '';
  ...
  async function setStatus(orderId, status, btn){
    const labelMap = {
      ...
      pronto_entrega: 'Pedido pronto saindo para entrega',
    ...
    try{
      ...
      let msg = 'Status do pedido foi atualizado.';
      if(status === 'recusado') msg = 'Pedido recusado e movido para “Recusados”.';
      if(status === 'em_fabricacao') msg = 'Pedido aceito e movido para “Em fabricação”.';
      if(status === 'pronto_entrega') msg = 'Pedido marcado como pronto para entrega.';
      if(status === 'finalizado'){
        const c = j && typeof j.commission !== 'undefined' ? Number(j.commission || 0) : COMMISSION_NOW;
        const tier = j && j.tier ? String(j.tier) : LEVEL_NAME;
        msg = 'Pedido entregue, finalizado e ' + moneyLabel(c) + ' liberadas no caixa (' + tier + ').';
        createFloatingCoin(c, btn);
      }

      removeRowFromUI(btn);

      const targetTab = getTargetTabForStatus(status);
      show(targetTab, true);

      toast('ok', 'Atualizado ✅', msg, 2600);

      setTimeout(function(){
        forceReloadToTab(targetTab);
      }, 250);
    } catch(err){
      console.error('[partners-ui] erro ao atualizar status:', err);
      ...
      lockRow(btn, false);
      toast('bad', 'Erro de conexão', 'Não foi possível sincronizar agora.', 3800);
    }
  }
  ...
  async function doCopyLink(){
    ...
    if(!inp){
      toast('bad', 'Link não encontrado', 'Abra a aba “Gerar link” e tente novamente.', 3200);
      return;
    }
  ...
  function toggleFocus(){
    ...
    toast(
      ...
      on ? 'Badges e animações reduzidas.' : 'Badges e feedbacks visuais voltaram.',
      1800
    );
  }
  ...
  function isValidEmailFormat(v){
    return /^[^\\s@]+@[^\\s@]+\\.[^\\s@]{2,}$/.test(String(v == null ? '' : v).trim().toLowerCase());
  }
  ...
  function validatePixKeyFront(type, key){
    const raw = String(key == null ? '' : key).trim();
    const digits = onlyDigits(raw);

    if(type === 'cpf'){
      if(!isDigitsOnly(raw)) return { ok:false, error:'A chave PIX CPF deve conter apenas números.' };
      if(digits.length !== 11) return { ok:false, error:'A chave PIX CPF deve ter 11 números.' };
      if(!isValidCPF(digits)) return { ok:false, error:'A chave PIX CPF é inválida.' };
      return { ok:true, normalized: digits };
    }

    if(type === 'cnpj'){
      if(!isDigitsOnly(raw)) return { ok:false, error:'A chave PIX CNPJ deve conter apenas números.' };
      if(digits.length !== 14) return { ok:false, error:'A chave PIX CNPJ deve ter 14 números.' };
      if(!isValidCNPJ(digits)) return { ok:false, error:'A chave PIX CNPJ é inválida.' };
      return { ok:true, normalized: digits };
    }

    if(type === 'telefone'){
      if(!isDigitsOnly(raw)) return { ok:false, error:'A chave PIX telefone deve conter apenas números.' };
      if(digits.length < 10 || digits.length > 13) return { ok:false, error:'A chave PIX telefone deve ter entre 10 e 13 números.' };
      return { ok:true, normalized: digits };
    }

    if(type === 'email'){
      if(!isValidEmailFormat(raw)) return { ok:false, error:'A chave PIX e-mail está inválida.' };
  ...
  function setWithdrawHint(msg){
    const el = document.getElementById('withdrawHint');
    if(!el) return;
    el.textContent = msg || '';
  ...
  function hasPix(){
    return !!normalizeText(PIX_STATE.pix_key) &&
           !!normalizeText(PIX_STATE.pix_type) &&
           !!normalizeText(PIX_STATE.pix_holder_name) &&
           !!normalizeText(PIX_STATE.pix_bank_name) &&
           !!normalizeText(PIX_STATE.pix_holder_document);
  }

  function syncWithdrawModal(){
    const fields = document.getElementById('withdrawPixFields');
    const intro = document.getElementById('withdrawIntroText');
    const alreadyHasPix = hasPix();

    if(fields){
      fields.style.display = alreadyHasPix ? 'none' : '';
    }

    if(intro){
      intro.textContent = alreadyHasPix
        ? 'Informe apenas a quantidade de moedas para solicitar o saque.'
        : 'Antes do primeiro saque, preencha seus dados PIX obrigatórios.';
    }
  }

  function applyPixInputRules(){
    const pixTypeInput = document.getElementById('withdrawPixType');
    const pixKeyInput = document.getElementById('withdrawPixKey');
    const holderCpfInput = document.getElementById('withdrawPixHolderDocument');

    if(holderCpfInput){
      holderCpfInput.addEventListener('input', function(){
        holderCpfInput.value = onlyDigits(holderCpfInput.value).slice(0, 11);
      });
    }

    function syncPixField(){
      if(!pixTypeInput || !pixKeyInput) return;

      const type = String(pixTypeInput.value || '').trim().toLowerCase();
      ...
      if(type === 'cpf'){
        pixKeyInput.value = onlyDigits(pixKeyInput.value).slice(0, 11);
        pixKeyInput.setAttribute('inputmode', 'numeric');
        pixKeyInput.setAttribute('maxlength', '11');
      } else if(type === 'cnpj'){
        pixKeyInput.value = onlyDigits(pixKeyInput.value).slice(0, 14);
        pixKeyInput.setAttribute('inputmode', 'numeric');
        pixKeyInput.setAttribute('maxlength', '14');
      } else if(type === 'telefone'){
        pixKeyInput.value = onlyDigits(pixKeyInput.value).slice(0, 13);
        pixKeyInput.setAttribute('inputmode', 'numeric');
        pixKeyInput.setAttribute('maxlength', '13');
      } else if(type === 'email'){
        pixKeyInput.setAttribute('maxlength', '180');
    ...
    if(pixTypeInput){
      pixTypeInput.addEventListener('change', syncPixField);
    }

    if(pixKeyInput){
      pixKeyInput.addEventListener('input', function(){
        const type = pixTypeInput ? String(pixTypeInput.value || '').trim().toLowerCase() : '';
  ...
  async function reloadWalletData(){
    ...
    if(j.pixState){
      PIX_STATE = {
        pix_key: j.pixState.pix_key || '',
        pix_type: j.pixState.pix_type || '',
        pix_holder_name: j.pixState.pix_holder_name || '',
        pix_bank_name: j.pixState.pix_bank_name || '',
        pix_holder_document: j.pixState.pix_holder_document || ''
      };
    }
    updateWalletUI();
    syncWithdrawModal();
  ...
  async function doWithdraw(){
    ...
    const pixTypeInput = document.getElementById('withdrawPixType');
    const pixKeyInput = document.getElementById('withdrawPixKey');
    const pixHolderNameInput = document.getElementById('withdrawPixHolderName');
    const pixBankNameInput = document.getElementById('withdrawPixBankName');
    const pixHolderDocumentInput = document.getElementById('withdrawPixHolderDocument');

    const amount = Number(amountInput ? amountInput.value : 0);
    const note = String(noteInput ? noteInput.value : '').trim();

    if(!Number.isFinite(amount) || amount <= 0){
      setWithdrawHint('Digite uma quantidade válida de moedas.');
      return;
    }
    ...
    if(!hasPix()){
      const pixType = normalizeText(pixTypeInput ? pixTypeInput.value : '').toLowerCase();
      const pixKey = normalizeText(pixKeyInput ? pixKeyInput.value : '');
      const holderName = normalizeText(pixHolderNameInput ? pixHolderNameInput.value : '');
      const bankName = normalizeText(pixBankNameInput ? pixBankNameInput.value : '');
      const holderCpf = onlyDigits(pixHolderDocumentInput ? pixHolderDocumentInput.value : '');

      if(!pixType){
        setWithdrawHint('Selecione o tipo de chave PIX.');
        return;
      }

      const pixCheck = validatePixKeyFront(pixType, pixKey);
      if(!pixCheck.ok){
        setWithdrawHint(pixCheck.error);
        return;
      }

      if(!holderName){
        setWithdrawHint('Informe o nome do titular da conta.');
        return;
      }

      if(!bankName){
        setWithdrawHint('Informe a instituição da conta.');
        return;
      }

      if(!holderCpf){
        setWithdrawHint('Informe o CPF do titular da conta.');
        return;
      }

      if(!isDigitsOnly(holderCpf)){
        setWithdrawHint('O CPF do titular deve conter apenas números.');
        return;
      }

      if(holderCpf.length !== 11){
        setWithdrawHint('O CPF do titular deve ter 11 números.');
        return;
      }

      if(!isValidCPF(holderCpf)){
        setWithdrawHint('O CPF do titular da conta é inválido.');
        return;
      }

      body.set('pix_type', pixType);
      body.set('pix_key', pixCheck.normalized);
      body.set('pix_holder_name', holderName);
      body.set('pix_bank_name', bankName);
      body.set('pix_holder_document', holderCpf);
    }
    ...
    try{
      ...
      if(!r.ok || !j || !j.ok){
        const map = {
          ...
          pix_holder_document_required: 'Informe o CPF do titular.',
          invalid_pix_key_email: 'A chave PIX do tipo e-mail está inválida.',
          invalid_pix_key_cpf: 'A chave PIX CPF é inválida.',
          invalid_pix_key_cpf_length: 'A chave PIX CPF deve ter 11 números.',
          invalid_pix_key_cpf_non_digits: 'A chave PIX CPF deve conter apenas números.',
          invalid_pix_key_cnpj: 'A chave PIX CNPJ é inválida.',
          invalid_pix_key_cnpj_length: 'A chave PIX CNPJ deve ter 14 números.',
          invalid_pix_key_cnpj_non_digits: 'A chave PIX CNPJ deve conter apenas números.',
          invalid_pix_key_phone: 'A chave PIX telefone está inválida.',
          invalid_pix_key_phone_non_digits: 'A chave PIX telefone deve conter apenas números.',
          invalid_holder_document_cpf: 'O CPF do titular da conta é inválido.',
          invalid_holder_document_cpf_length: 'O CPF do titular deve ter 11 números.',
          invalid_holder_document_cpf_non_digits: 'O CPF do titular deve conter apenas números.'
        };

        setWithdrawHint(map[(j && j.error) || ''] || ((j && j.error) ? j.error : 'Tente novamente.'));
        return;
      }

      if(j.pix){
        PIX_STATE = {
          pix_key: j.pix.pix_key || '',
          pix_type: j.pix.pix_type || '',
          pix_holder_name: j.pix.pix_holder_name || '',
          pix_bank_name: j.pix.pix_bank_name || '',
          pix_holder_document: j.pix.pix_holder_document || ''
        };
      }
      ...
      if(pixTypeInput) pixTypeInput.value = '';
      if(pixKeyInput) pixKeyInput.value = '';
      if(pixHolderNameInput) pixHolderNameInput.value = '';
      if(pixBankNameInput) pixBankNameInput.value = '';
      if(pixHolderDocumentInput) pixHolderDocumentInput.value = '';

      toast('ok', 'Saque registrado', 'Sua solicitação foi salva com sucesso.', 2800);
    }catch(err){
      console.error('[wallet-ui] erro withdraw:', err);
      setWithdrawHint('Não foi possível registrar o saque agora.');
    }
  }
  ...
  document.addEventListener('click', function(ev){
    ...
    if(actionEl){
      ...
      if(action === 'setStatus'){
        ...
        if(id && st) setStatus(id, st, actionEl);
        return;
      }
...
function normalizePixState(pix) {
  return {
    pix_key: String(pix?.pix_key || ""),
    pix_type: String(pix?.pix_type || ""),
    pix_holder_name: String(pix?.pix_holder_name || ""),
    pix_bank_name: String(pix?.pix_bank_name || ""),
    pix_holder_document: String(pix?.pix_holder_document || ""),
  };
}